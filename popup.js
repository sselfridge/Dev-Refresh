...
on_off_button.onclick = function(element) {
  chrome.storage.sync.get(["activeTabs"], function(result) {
    //fetch active tabs from storage
    let activeTabs = result["activeTabs"];

    currentTabActive = activeTabs.includes(currentTab);

    console.debug(
      `Popup.js----Active Tabs ${activeTabs} CurrentTabActive?:${currentTabActive}`
    );
    ...
    if (currentTabActive) {
      ...
      //turn extention on for current tab
...
// Show / Hide local files link if needed

chrome.extension.isAllowedFileSchemeAccess(localAllowed => {
  if(!localAllowed){
